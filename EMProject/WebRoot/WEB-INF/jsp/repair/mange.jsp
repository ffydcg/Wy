...
<script type="text/javascript">
	var ctx = "${ctx}";
	function modify(id) {
		window.location.href = ctx + "/toRepairMdf.do?id=" + id;
	}
	function del(id) {
		var ctx = "${ctx}"
		$.ajax({
			...
			success : function(msg) {
				if (msg == '0000') {
					alert("业主删除成功");
					window.location.href = ctx +"/listRepair.do";
				} else {
					alert("业主删除失败");
					window.location.href = ctx +"/listRepair.do";
				}
	...
	window.location.href = ctx + "/listRepairByNum.do?num=" + num;
	}
	function queryInfo(){
	var name=$("#queryName").val().trim();
	var url=ctx +"/queryRepairByName.do?name="+name;
	...
	<body>
		...
		<!--<hr id="A"/>-->
		<div style="height: 655px;width: 300px;position: absolute;top: 100px; background-color:#F7FFFF ;opacity: 0.8;">
		...
		<div id="a">
			<table style="position: absolute;top: 139px;left: 150px;">
				...
				<caption style="font-size: 24px;color: #000000;position: absolute;top: -60px;lleft: 4px;">维修列表:</caption>
				<thead>
					<td style="width: 100px;">业主编号</td>
					<td style="width: 120px;">业主姓名</td>
					<td style="width: 100px;">维修部位</td>
					<td style="width: 150px;">维修费用</td>
					<td style="width: 150px;">费用缴纳情况</td>
				...
				<tr class="a">
					<td>${repair.repairOwnerNo}</td>
					<td>${repair.repairOwnerName}</td>
					<td>${repair.repairContent}</td>
					<td>${repair.repairFee}</td>
					<td>${repair.repairFeeStatus }</td>
					<td><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="modify('${repair.id}')">修改</a><span>|</span><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="del('${repair.id}')">删除</a></td>
				</tr>
				</c:forEach>
				</tbody>
			...
			<a href="${ctx}/toRAdd.do" style="position: absolute;top: 100px;right: 250px" href="æ·»å ä¸ä¸»èµæ.html"><img src="${ctx }/image/add.png"></a>
		</div>
