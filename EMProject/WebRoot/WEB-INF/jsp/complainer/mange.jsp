...
<script type="text/javascript">
	var ctx = "${ctx}";
	function modify(id) {
		window.location.href = ctx + "/toComplainerMdf.do?id=" + id;
	}
	function del(id) {
		var ctx = "${ctx}"
		$.ajax({
			...
			success : function(msg) {
				if (msg == '0000') {
					alert("业主删除成功");
					window.location.href = ctx +"/listComplainer.do";
				} else {
					alert("业主删除失败");
					window.location.href = ctx +"/listComplainer.do";
				}
	...
	window.location.href = ctx + "/listComplainerByNum.do?num=" + num;
	}
	function queryInfo(){
	var name=$("#queryName").val().trim();
	var url=ctx +"/queryComplainerByName.do?name="+name;
	...
	<body>
		...
		<!--<hr id="A"/>-->
		<div style="height: 661px;width: 300px;position: absolute;top: 100px; background-color:#F7FFFF ;opacity: 0.8;">
		...
		<div id="a">
			<table style="position: absolute;top: 139px;left: 40px;">
				...
				<caption style="font-size: 24px;color: #000000;position: absolute;top: -60px;left: 4px;">投诉列表:</caption>
				<thead>
					<td style="width: 100px;">投诉编号</td>
					<td style="width: 150px;">投诉业主</td>
					<td style="width: 100px;">接待人员</td>
					<td style="width: 150px;">投诉日期</td>
					<td style="width: 150px;">处理日期</td>
					<td style="width: 180px;">投诉内容</td>
					<td style="width: 150px;">操作</td>

				</thead>
				<tbody>
				<c:forEach items="${page.list}" var="complainer">
				<tr class="a">
					<td>${complainer.id}</td>
					<td>${complainer.comOwnerName}</td>
					<td>${complainer.comAccepter}</td>
					<td><fmt:formatDate value="${complainer.comDate}" pattern="yyyy-MM-dd"/></td>
					<td><fmt:formatDate value="${complainer.handleDate}" pattern="yyyy-MM-dd"/></td>
					<td>${complainer.comContent}</td>
					<td><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="modify('${complainer.id}')">修改</a><span>|</span><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="del('${complainer.id}')">删除</a></td>
				</tr>
				</c:forEach>
				</tbody>
			...
			<a href="${ctx}/toCAdd.do" style="position: absolute;top: 100px;right: 250px"><img src="${ctx }/image/add.png"></a>
		</div>
