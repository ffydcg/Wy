...
<script type="text/javascript">
	var ctx = "${ctx}";
	function modify(id) {
		window.location.href = ctx + "/townerMdf.do?id=" + id;
	}
	function del(id) {
		var ctx = "${ctx}"
		$.ajax({
			...
			success : function(msg) {
				if (msg == '0000') {
					alert("业主删除成功");
					window.location.href = ctx +"/listOwner.do";
				} else {
					alert("业主删除失败");
					window.location.href = ctx +"/listOwner.do";
				}
	...
	window.location.href = ctx + "/listOwnerByNum.do?num=" + num;
	}
	function queryInfo(){
	var name=$("#queryName").val().trim();
	var url=ctx +"/queryOwnerByName.do?name="+name;
	...
	<body>
		...
		<!--<hr id="A"/>-->
		<div style="height: 671px;width: 300px;position: absolute;top: 99px; background-color:#F7FFFF ;opacity: 0.8;">
		...
		<div id="a">
			<table style="position: absolute;top: 139px;left: 40px;">
				...
				<caption style="font-size: 24px;color: #000000;position: absolute;top: -60px;left: 4px;">业主资料:</caption>
				<thead>
					<td style="width: 130px;">业主编号</td>
					<td style="width: 100px;">姓名</td>
					<td style="width: 140px;">联系电话</td>
					<td style="width: 170px;">住房信息</td>
					<td style="width: 150px;">入住日期</td>
					<td style="width: 100px;">住房面积</td>
				...
				<tr class="a">
					...
					<td>${owner.ownerHouseInfo}</td>
					<td><fmt:formatDate value="${owner.ownerCheckinDate}" pattern="yyyy-MM-dd"/></td>
					<td>${owner.ownerHouseArea }</td>
					<td><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="modify('${owner.id}')">修改</a><span>|</span><a style="color: blue;font-size: 18px;" href="javascript:void(0);" onclick="del('${owner.id}')">删除</a></td>
				</tr>
				</c:forEach>
				</tbody>
			...
			<a href="${ctx}/toOAdd.do" style="position: absolute;top: 100px;right: 250px" href="æ·»å ä¸ä¸»èµæ.html"><img src="${ctx }/image/add.png"></a>
		</div>
